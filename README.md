...
- **feature/customizable-assets**（当前分支）：在 main 的基础上，加了"不用重新编译，直接换 exe 旁边 assets 文件夹里的图就能换皮肤"的功能，见下面"换皮肤不用重新编译"一节。
...
│   ├── index.html
│   ├── renderer.js
│   └── assets/              默认宠物素材(idle/walk1/walk2/walk3/drag.png)
├── src-tauri/                Rust 后端
...
（这套"外部 assets 文件夹优先，缺什么就用现有的凑，图不全也不强制回退默认图"的逻辑在 `src-tauri/src/skin.rs` 的 `resolve_skin_config` 函数里，想改判定规则改这里就行。）
